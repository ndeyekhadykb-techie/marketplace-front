    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {[
            { label: 'Revenus totaux', value: `${parseFloat(stats?.total_revenue || 0).toLocaleString()} FCFA`, icon: '💰', bg: 'bg-green-50', text: 'text-green-600' },
            { label: 'Commandes totales', value: stats?.total_orders || 0, icon: '🧾', bg: 'bg-blue-50', text: 'text-blue-600' },
            { label: 'Produits vendus', value: stats?.total_products_sold || 0, icon: '📦', bg: 'bg-orange-50', text: 'text-orange-600' },
            { label: 'Note moyenne', value: stats?.average_rating ? `${parseFloat(stats.average_rating).toFixed(1)} ⭐` : 'N/A', icon: '⭐', bg: 'bg-yellow-50', text: 'text-yellow-600' },
          ].map((card, i) => (
              ...
              {stats.top_products.map((product, i) => (
                <div key={i} className="flex items-center justify-between py-3 border-b border-gray-50 last:border-0">
                  <div className="flex items-center gap-3">
                    <span className="w-7 h-7 bg-red-50 text-red-500 rounded-full flex items-center justify-center text-xs font-bold">
                      {i + 1}
                    </span>
